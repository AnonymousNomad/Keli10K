"""Typewriter renderer with DNA helix + underwater syntax highlighting."""
import sys, time, re, json, urllib.request, urllib.error

class ResponseRenderer:
    SYNTAX_MAP = {
        r'(def |class |import |from |return|if |elif |else:|for |while |try:|except|with |async |await )': '\033[1;94m',
        r'("[^"]*"|\'[^\']*\')': '\033[92m',
        r'(#.*)': '\033[90m',
        r'(\b\d+\.?\d*\b)': '\033[93m',
        r'(```[\s\S]*?```)': None,
    }

    LANG_MAP = {
        'py': '\033[93mPython\033[0m', 'js': '\033[92mJavaScript\033[0m',
        'ts': '\033[94mTypeScript\033[0m', 'css': '\033[95mCSS\033[0m',
        'html': '\033[91mHTML\033[0m', 'json': '\033[93mJSON\033[0m',
        'bash': '\033[90mBash\033[0m'
    }

    def __init__(self, api_url='http://localhost:8080'):
        self.api_url = api_url

    def render_keli_response(self, lines, typewriter_delay=0.008):
        sys.stdout.write('\033[1;95m◇ [FSI Keli]:\033[0m ')
        sys.stdout.flush()
        in_code = False
        code_lang = ''
        for i, line in enumerate(lines):
            if line.startswith('```'):
                if in_code:
                    in_code = False
                    sys.stdout.write('\033[0m\n')
                    sys.stdout.write(f'  \033[36m╲╱\033[0m\n')
                else:
                    in_code = True
                    code_lang = line[3:].strip()
                    hl = self.LANG_MAP.get(code_lang, f'\033[96m{code_lang}\033[0m')
                    sys.stdout.write(f'\n  \033[92m├─[{hl}]─\033[0m\n')
                sys.stdout.flush()
                continue
            if in_code:
                sys.stdout.write(f'  \033[92m│\033[0m {line}\n')
            elif line.startswith('[') and ']' in line:
                sys.stdout.write(f'\033[2;96m{line}\033[0m\n')
            else:
                for char in line:
                    sys.stdout.write(char)
                    sys.stdout.flush()
                    time.sleep(typewriter_delay)
                sys.stdout.write('\n')
            sys.stdout.flush()
        if in_code:
            sys.stdout.write(f'  \033[92m└─ ~≈≈~~\033[0m\n\033[0m')
        sys.stdout.write('\n')
        sys.stdout.flush()

    def render_build_output(self, files):
        sys.stdout.write('\n\033[1;95m◇ [FSI Keli]:\033[0m \033[92mDone.\033[0m Your project:\n')
        for name, size in files:
            sys.stdout.write(f'  \033[92m├─\033[0m \033[93m📄\033[0m {name}  (\033[94m{size})\033[0m\n')
        sys.stdout.write(f'  \033[92m└──╼ \033[90m0101  ~≈≈~~  ◇\033[0m\n')
        sys.stdout.flush()
